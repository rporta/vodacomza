...
helper = {
    ...
    chargeRequestWhitEncriptedMsisdn: (data, callback) => {

        var parameters = {
            'partner-id': data.partnerId,
            'token': data.token,
            'package-id': data.packageId,
            'client-txn-id': data.clientTxnId,
            'partner-redirect-url': data.partnerRedirectUrl,
        };
        ...
        var postRequest = {
            host: this.operator.mw.host,
            port: 8080,
            path: "/enterprise-services/ppd/service/partner/verify/v1?" + queryString,
            timeout: this.operator.mw.timeoutMsec,
            auth: this.operator.mw.user + ':' + this.operator.mw.password,
            method: "GET",
            headers: {
